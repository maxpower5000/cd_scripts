...
try
{
    $issues = Invoke-RestMethod -Method POST -Uri "$jiraUrl/rest/api/latest/search" -Body $JsonBody -Headers @{"Authorization"="Basic $EncodedCredentials"} `
...
try
{
    if($attachPdfReport)
    {
        Send-MailMessage -From $From -To $To -Subject $Subject -Body $emailData -BodyAsHtml -SmtpServer $SmtpServer -Attachments "$PSScriptRoot\$pdfReport" `
            -Encoding $([System.Text.Encoding]::UTF8) -ErrorAction Stop
    }
    else
    {
        Send-MailMessage -From $From -To $To -Subject $Subject -Body $emailData -BodyAsHtml -SmtpServer $SmtpServer `
            -Encoding $([System.Text.Encoding]::UTF8) -ErrorAction Stop
    }
}
catch
